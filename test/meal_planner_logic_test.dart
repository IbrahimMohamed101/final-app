...
void main() {
  ...
  setUp(() async {
    ...
    await bloc.stream.firstWhere(
      (s) => s is MealPlannerLoaded && s.maxMeals > 0,
    );
  });
  ...
  group('MealPlannerBloc Dynamic Slots Logic', () {
    ...
    test('tapping Add Meal adds a second card', () {
      ...
      return expectLater(
        bloc.stream,
        emitsThrough(
          predicate<MealPlannerState>((state) {
    ...
    test('removing extra slot re-indexes remaining slots', () async {
      ...
      // Wait for add
      await bloc.stream.firstWhere(
        (s) => s is MealPlannerLoaded && s.maxMeals == 2,
      );

      bloc.add(const RemoveMealSlotEvent(1)); // Remove 2nd slot (index 1)

      await bloc.stream.firstWhere(
        (s) => s is MealPlannerLoaded && s.maxMeals == 1,
      );
    ...
    test('missing mealBalance falls back to requiredMeals and enforces limit',
        () async {
      ...
      await fallbackBloc.stream.firstWhere(
        (s) => s is MealPlannerLoaded && s.maxMeals > 0,
      );
